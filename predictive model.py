...
def build_model_input():
    ...
    gc.collect()
    #left join bureau and bureau balance
    print('Merge with buro avg')
    buro_full = buro_full.merge(right=avg_buro_bal.reset_index(), how='left', on='SK_ID_BUREAU', suffixes=('', '_bur_bal'))
    # Bureau: Perform aggregations
    ...
    del prev_dum
    gc.collect()
    # previous-application: Perform aggregations
    ...
    # credit card balance: Perform aggregations
    ...
    # installments: Perform aggregations
    ...
    #join training and testing data with calculated averages
...
def train_model(data_, test_, y_, folds_):
    #remove special JSON characters in feature name
    data_.columns = ["".join (c if c.isalnum() else "_" for c in str(x)) for x in data_.columns]
    test_.columns = ["".join (c if c.isalnum() else "_" for c in str(x)) for x in test_.columns]
    oof_preds = np.zeros(data_.shape[0])
    sub_preds = np.zeros(test_.shape[0])
    #initialize data frame for feature performance
